...
*/
SELECT emp_id, emp_name, dept_code, salary FROM employee WHERE salary >= 3_000_000;

/* Q5)
부서코드가 D6이고 급여를 200만원보다 많이 받는 직원의 이름, 부서코드, 급여를 조회하시오.
*/
SELECT emp_name, dept_code, salary FROM employee WHERE dept_code = 'D6' AND salary > 2_000_000;

/* Q6)
보너스를 받지 않는 사원에 대해 사번, 직원명, 급여, 보너스를 조회하시오.
*/
SELECT emp_id, emp_name, salary, bonus FROM employee WHERE bonus ISNULL;
...
*/
SELECT emp_no, emp_name, salary, dept_code, job_code FROM employee WHERE salary BETWEEN 3_500_000 AND 5_500_000;
SELECT emp_no, emp_name, salary, dept_code, job_code FROM employee WHERE salary >= 3_500_000 AND salary <= 5_500_000;

/* Q10)
employee 테이블에서 350만원 미만, 또는 550만원을 초과하는 직원의 사번, 이름, 부서코드, 급여를 조회하시오.
*/
SELECT emp_no, emp_name, dept_code, salary  FROM employee WHERE salary < 3_500_000 OR salary > 5_500_000;


/* Q11)
employee 테이블에서 성이 김씨인 직원의 사번, 이름, 입사일을 조회하시오.
*/
SELECT emp_no, emp_name, hire_date FROM employee WHERE emp_name LIKE '김%';

/* Q12)
employee 테이블에서 성이 김씨 성이 아닌 직원의 사번, 이름, 입사일을 조회하시오.
*/
SELECT emp_no, emp_name, hire_date FROM employee WHERE NOT emp_name LIKE '김%';

/* Q13)
employee 테이블에서 '하'가 이름에 포함된 직원의 이름, 주민번호, 부서코드를 조회하시오.
*/
SELECT emp_no, emp_name, dept_code FROM employee WHERE emp_name LIKE '%하%';
...
(힌트: '_')
*/
SELECT emp_id, emp_name, phone FROM employee WHERE phone LIKE '%9_______';
...
*/
SELECT emp_no, emp_name, email FROM employee WHERE email LIKE '___!_%' ESCAPE '!';
...
*/
SELECT * FROM employee WHERE NOT dept_code IN ('D6', 'D8');
...
FROM employee
WHERE (job_code = 'J2' AND salary >= 2_000_000) OR job_code = 'J7';
...
SELECT emp_name, salary, job_code
FROM employee
WHERE (job_code = 'J2' OR job_code = 'J7') AND salary >= 2_000_000;