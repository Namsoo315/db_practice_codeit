...
SELECT *
FROM employee
WHERE emp_id = (SELECT manager_id::INT FROM employee WHERE emp_name = '전지현');

-- 평균 급여보다 많은 사람 찾기 (조인으로 해결불가)
SELECT AVG(employee.salary)
FROM employee;
SELECT *
FROM employee
WHERE salary > 2792656;
SELECT *
FROM employee
WHERE salary > (SELECT AVG(employee.salary) FROM employee);

-- 평균 급여보다 많이 받는 사람들 이름과 부서이름까지 출력해라 (subquery + join)
...
SELECT *
FROM employee
WHERE EXISTS (SELECT dept_code FROM employee WHERE dept_code = 'D11');
SELECT emp_id,
       emp_name,
       salary,
       dept_code
...
-- GROUP BY 절 -> 부서별 평균 월급 TOP3인 부서
SELECT RANK() OVER (ORDER BY AVG(salary) DESC ) AS 순위,
       dept_code,
       FLOOR(AVG(salary))                       AS 팀평균월급
FROM employee
...
SELECT e1.emp_name,
       ...
       top3.순위
FROM employee e1
         JOIN (SELECT RANK() OVER (ORDER BY AVG(salary) DESC ) AS 순위,
                      dept_code,
                      FLOOR(AVG(salary))                       AS 팀평균월급
               FROM employee
               WHERE dept_code IS NOT NULL
...
--( SELECT AVG(salary)  FROM employee e2  WHERE e1.dept_code = e2.dept_code) -> 그룹 함수로 전화

--부서별 평균 그룹바이절
SELECT employee.dept_code, AVG(employee.salary)
FROM employee
GROUP BY dept_code;
...
SELECT e.emp_name,
       ...
       FLOOR(d.부서별평균)
FROM employee e
         JOIN (SELECT employee.dept_code, AVG(employee.salary) AS 부서별평균
               FROM employee
