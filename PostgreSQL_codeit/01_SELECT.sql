...
-- * : 와일드 카드, 모든 컬럼을 조회하는 키워드, 급할때만 활용 권장!
...
-- 컬럼명을 지정하여 조회하는 방법
-- -> 일반적으로 DB 값을 조횧하기 위해선 컬럼명을 명시하는것이 정석
...
FROM employee;
SELECT employee.emp_id, employee.emp_name, employee.emp_no
FROM employee;
...
-- AS 문법 : table이나 컬럼명의 별칭을 만드는 방법, 향후 Java와 연동시 핵심적으로 사용 될 문법 ★★★★★
-- 컬렴명 AS "별칭", AS 별칭, 별칭
SELECT emp_name
FROM employee;
...
-- PK 기준으로 조회하는 방법, WHERE절에서 가장 많이 쓰이는 문법
SELECT *
FROM employee
WHERE emp_id = 200;
SELECT *
...
SELECT emp_name, salary, dept_code
FROM employee
WHERE dept_code = 'd1'; --여기서는 대소문자가 민감함!
SELECT emp_name, salary, dept_code
FROM employee
...
FROM employee
WHERE dept_code = 'D1'
  AND salary >= 3_000_000;

-- 직원중 부서가 D1이면서 월급이 200이상 300이하인 사람 -> BETWEEN
SELECT emp_name, salary, dept_code
FROM employee
WHERE dept_code = 'D1'
  AND salary BETWEEN 2_000_000 AND 3_000_000;

SELECT emp_name, salary, dept_code
FROM employee
WHERE dept_code = 'D1'
  AND salary <= 3_000_000
  AND salary >= 2_000_000;
...
-- 사용법
-- WHERE '컬럼명' LIKE '문자%' : 앞의  두글자로 시작로 시작하고 가운데 '문자'라는 키워드가 들어있는 아무 문자열 탐색
...
SELECT emp_name
FROM employee
WHERE emp_name LIKE '이%'; -- 이씨성을 가진 사람
SELECT emp_name
FROM employee
WHERE emp_name LIKE '이__'; -- 이씨성을 가진 3글자 이름인 사람
SELECT emp_name
FROM employee
WHERE emp_name LIKE '이_'; -- 이씨성을 가진 2글자 이름인 사람
SELECT emp_name
FROM employee
...
SELECT *
FROM employee
WHERE email LIKE '____%'; -- 실패, 4글자 이상인 사람
SELECT *
FROM employee
...
-- SELECT emp_name, bonus FROM employee WHERE bonus = NULL; --문법은 먹히나 의도와는 다름!
SELECT emp_name, bonus
FROM employee
WHERE bonus ISNULL; -- 정상
SELECT emp_name, bonus
FROM employee
WHERE bonus IS NOT NULL; -- 정상
SELECT emp_name, bonus
FROM employee
WHERE NOT bonus ISNULL;
-- 정상

-- 컬럼의 NULL이 있지만 특정 연산을 수행하고 싶은 경우 -> 지급될 월급과 보너스를 구해라!
SELECT emp_name, salary AS 월급
FROM employee;
SELECT emp_name, salary AS 월급, salary + (salary * bonus) AS 지급급액
FROM employee;
-- NULL이 묻으면 NULL이 된다.
--> NULL 체크 및 변환을 수행하지 않아 생긴 문제, NVL 처리가 필요.

--NULL 체크 문법 -> DB마다 방언이 존재하나 표준 함수도 존재
-- coalesce(ANSI 표준) : NULL을 특정을 값으로 변환하는 함수
...
-- 문자
SELECT emp_name FROM employee ORDER BY emp_name; -- 사전순 정렬, DEFAULT는 ASC(오름차순)
SELECT emp_name FROM employee ORDER BY emp_name ASC; -- 명시적 표기 방법
SELECT emp_name FROM employee ORDER BY emp_name DESC; -- 명시적 표기 방법
...
-- 컬렴 명이 아닌 출력 순서로 정하는 방법 (index), 권장하지 않는다!!!
--       1         2
SELECT emp_id, emp_name FROM employee ORDER BY 1 DESC;  -- emp_id 내림차순
SELECT emp_id, emp_name FROM employee ORDER BY 2 DESC;  -- emp_name 내림차순

-- 컬럼며이 아닌 별명으로 정렬하기 -> 함수나 복잡한 컬럼명을 별명으로 축약가능
SELECT employee.emp_id, employee.emp_name AS 이름 FROM employee ORDER BY 이름 DESC;
SELECT employee.emp_name, emp_no, SUBSTRING(emp_no FROM 8 FOR 1) AS 성별
FROM employee
ORDER BY 성별;
